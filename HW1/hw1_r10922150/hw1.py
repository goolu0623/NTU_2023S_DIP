...
def problem_1_a():
    ...
    row, column, channel = sample2.shape
    result3 = np.zeros((row, column, 3), np.uint8)
    for rownum in range(row):
        for colnum in range(column):
            result3[rownum, colnum] = sample2[rownum, colnum] / 3
...
def problem_1_f():
    ...
    sample2 = cv2.imread("./hw1_sample_images/sample2.png",
                         cv2.IMREAD_GRAYSCALE)
    temp = local_hist_equalization(sample2, 100)
    row,col = temp.shape
    for rownum in range(row):
        for colnum in range(col):
            temp[rownum,colnum] = abs(255-temp[rownum,colnum])
    cv2.imwrite("result11.png",temp)
...
def print_hist():
    ...
    # image1 = cv2.imread("./hw1_sample_images/sample2.png",
    #                      cv2.IMREAD_GRAYSCALE)
    image1 = cv2.imread("result5.png", cv2.IMREAD_GRAYSCALE)
    image2 = cv2.imread("result6.png", cv2.IMREAD_GRAYSCALE)
    image3 = cv2.imread("result7.png", cv2.IMREAD_GRAYSCALE)
    ...
    ax1.title.set_text("result5 hist")
    ax1.hist(image1.ravel(), bins=256)
    ax1.set_xlim([0, 255])

    ax2.title.set_text("result6 hist")
    ax2.hist(image2.ravel(), bins=256)
    ax2.set_xlim([0, 255])

    ax3.title.set_text("result7 hist")
    ax3.hist(image3.ravel(), bins=256)
    ax3.set_xlim([0, 255])

    plt.savefig("result567_compare.png")
